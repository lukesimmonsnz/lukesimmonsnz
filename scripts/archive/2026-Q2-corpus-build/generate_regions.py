...
# Regional configs
REGIONS = {
    "nelson": {"slug": "nelson", "dir": "nelson", "display": "Nelson — Whakatū"},
    "tasman": {"slug": "tasman", "dir": "tasman", "display": "Tasman — Te Tai-o-Aorere"},
    "west-coast": {"slug": "west-coast", "dir": "west-coast", "display": "West Coast — Te Tai Poutini"},
}
...
THEMES = {
    ...
    "transport": ["connectivity", "nelson_congestion", "airport_capacity", "active_modes"],
    "infrastructure": ["infrastructure", "wastewater", "water", "port"],
    ...
    "economy": ["economic_structure", "horticulture", "tourism", "economic_diversification"],
    "governance": ["governance", "growth_management", "treaty", "unitary_capacity"],
    "climate": ["climate_risk", "sea_level_rise", "fire_risk", "flooding"],
}

def write_problem(region_id, region_slug, region_dir, problem_id, parent_id, theme, order):
    """Write a problem YAML file."""
    path = Path(f"content/{region_dir}/data/problem") / f"{theme}.{problem_id}.yaml"
    path.parent.mkdir(parents=True, exist_ok=True)

    data = {
        "id": f"problem.{region_id}.{theme}.{problem_id}",
        "title": f"Problem: {problem_id.replace('_', ' ').title()} in {region_id}",
        "theme": theme,
        ...
        "order": order,
        "updated": "2026-04-26",
        "summary": f"Description of {problem_id} in {region_id}.",
...
def write_claim(region_id, region_slug, claim_id, theme, problem_id):
    ...
    path = Path(f"content/{region_id}/data/claim") / f"{theme}.{claim_id}.yaml"
    path.parent.mkdir(parents=True, exist_ok=True)

    data = {
        "id": f"claim.{region_id}.{theme}.{claim_id}",
        "statement": f"Claim regarding {claim_id.replace('_', ' ')} in {region_id}.",
        "value": None,
        "unit": None,
...
def write_driver(region_id, region_slug, driver_id, theme, problem_ids):
    ...
    path = Path(f"content/{region_id}/data/driver") / f"{theme}.{driver_id}.yaml"
    path.parent.mkdir(parents=True, exist_ok=True)

    data = {
        "id": f"driver.{region_id}.{theme}.{driver_id}",
...
def write_camp(region_id, region_slug, camp_id, theme, problem_ids):
    ...
    path = Path(f"content/{region_id}/data/camp") / f"{theme}.{camp_id}.yaml"
    path.parent.mkdir(parents=True, exist_ok=True)

    data = {
        "id": f"camp.{region_id}.{theme}.{camp_id}",
...
for region_id, region_config in REGIONS.items():
    ...
    for theme, problem_list in THEMES.items():
        ...
        # Write root problem
        write_problem(region_id, region_slug, root_problem, None, theme, 1)

        # Write child problems
        for order, child_problem in enumerate(children, 2):
            write_problem(region_id, region_slug, child_problem, root_problem, theme, order)

        # Write 1 claim per problem (root + children)
        for problem in problem_list:
            claim_id = f"{problem}_claim"
            write_claim(region_id, region_slug, claim_id, theme, f"problem.{region_id}.{theme}.{problem}")

        # Write 2 drivers per theme
        for driver_idx in range(1, 3):
            driver_id = f"driver_{driver_idx}"
            problem_ids = [
                f"problem.{region_id}.{theme}.{root_problem}",
                f"problem.{region_id}.{theme}.{children[driver_idx - 1] if driver_idx <= len(children) else children[0]}"
            ]
        ...
        for camp_idx in range(1, 3):
            ...
            problem_ids = [
                f"problem.{region_id}.{theme}.{root_problem}",
                f"problem.{region_id}.{theme}.{children[camp_idx - 1] if camp_idx <= len(children) else children[0]}"
            ]
            write_camp(region_id, region_slug, camp_id, theme, problem_ids)

print("Generated Nelson, Tasman, West Coast corpora")
